...
#define NVT_HASH_DMA_SRC_REG            0x94           ///< source dma address, word alignment
#define NVT_HASH_DMA_DST_REG            0x98           ///< destination dma address, word alignment
...
static void nvt_hash_platform_init(void)
{
	...
	/* Hash clock select, 0:240MHz 1:320MHz 2: RVD 3: PLL9 */
...
int nvt_hash_pio_sha(struct nvt_hash_pio_t *p_hash)
{
	...
	hash_dbg("Src     => addr:0x%08x size:%u\n", (uint32_t)p_hash->src,    p_hash->src_size);
	hash_dbg("Digest  => addr:0x%08x size:%u\n", (uint32_t)p_hash->digest, p_hash->digest_size);

	/* check source buffer and size, the zero source size is valid */
	if (!p_hash->src && (p_hash->src_size > 0)) {
		hash_err("source buffer=0x%08x size:%u invalid\n", (uint32_t)p_hash->src, p_hash->src_size);
		return -1;
	}

	/* check digest buffer and size */
	if (!p_hash->digest || (p_hash->digest_size < out_size)) {
		hash_err("digest buffer=0x%08x size:%u invalid\n", (uint32_t)p_hash->digest, p_hash->digest_size);
		return -1;
	}
...
int nvt_hash_dma_sha(struct nvt_hash_pio_t *p_hash)
{
	...
	/* check source buffer and size, the zero source size is valid */
	if ((!p_hash->src && (p_hash->src_size > 0)) || (p_hash->src && (((uint32_t)p_hash->src)&0x1f))) {
		hash_err("source buffer=0x%08x size:%u invalid\n", (uint32_t)p_hash->src, p_hash->src_size);
		return -1;
	}
	...
	hash_write(NVT_HASH_DMA_SRC_REG,     (uint32_t)p_hash->src);
