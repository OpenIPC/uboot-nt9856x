...
static void disp_device_init(T_LCD_PARAM *p_mode)
{
	...
	//T_LCD_SIF_PARAM         *dev_sif_param; SIF_bypass
#if (DISPLCDSEL_IF8BITS_TYPE == DISPLCDSEL_IF8BITS_LCD1)
	...
	dev_io_ctrl.SEL.GET_REG_IF.lcd_ctrl = DISPDEV_LCDCTRL_GPIO;
	/*if (dev_io_ctrl.SEL.GET_REG_IF.lcd_ctrl == DISPDEV_LCDCTRL_SIF) { SIF_bypass
...
static void dispdev_write_lcd_reg(UINT32 ui_addr, UINT32 ui_value)
{
	...
	} /*else if (dev_io_ctrl.SEL.GET_REG_IF.lcd_ctrl == DISPDEV_LCDCTRL_SIF) { SIF_bypass
