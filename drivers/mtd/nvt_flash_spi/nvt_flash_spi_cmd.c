			...
			if(uiColAddr == info->flash_info->page_size) {
				...
				if(uiNandSectionCountPerPage > 1) {
					for(j = 0; j < uiNandSectionCountPerPage; j++) {
						*(pulBuf + j +  0) = *(uint32_t *)(uiSpareAreaAddr+ 0 + j * 4);
						*(pulBuf + j +  4) = *(uint32_t *)(uiSpareAreaAddr+16 + j * 4);
						*(pulBuf + j +  8) = *(uint32_t *)(uiSpareAreaAddr+32 + j * 4);
						*(pulBuf + j + 12) = *(uint32_t *)(uiSpareAreaAddr+48 + j * 4);
					}
				} else {
					*(pulBuf + 0) = *(uint32_t *)(uiSpareAreaAddr+0x0);
					*(pulBuf + 1) = *(uint32_t *)(uiSpareAreaAddr+0x4);
					*(pulBuf + 2) = *(uint32_t *)(uiSpareAreaAddr+0x8);
					*(pulBuf + 3) = *(uint32_t *)(uiSpareAreaAddr+0xC);
				}
				...
				NAND_SETREG(info, NAND_DMASTART_REG_OFS, dma_getPhyAddr((uintptr_t)pulBuf));
				...
				if(uiNandSectionCountPerPage > 1) {
					for(j = 0; j < uiNandSectionCountPerPage; j++) {
						*(pulBuf_spare + j +  0) = *(uint32_t *)(uiSpareAreaAddr+ 0 + j * 4);
						*(pulBuf_spare + j +  4) = *(uint32_t *)(uiSpareAreaAddr+16 + j * 4);
						*(pulBuf_spare + j +  8) = *(uint32_t *)(uiSpareAreaAddr+32 + j * 4);
						*(pulBuf_spare + j + 12) = *(uint32_t *)(uiSpareAreaAddr+48 + j * 4);
					}
				} else {
					*(pulBuf_spare + 0) = *(uint32_t *)(uiSpareAreaAddr+0x0);
					*(pulBuf_spare + 1) = *(uint32_t *)(uiSpareAreaAddr+0x4);
					*(pulBuf_spare + 2) = *(uint32_t *)(uiSpareAreaAddr+0x8);
					*(pulBuf_spare + 3) = *(uint32_t *)(uiSpareAreaAddr+0xC);
				}
...
int spinor_read_operation(struct drv_nand_dev_info *info, u32 byte_addr, u32 byte_size, u8 *pbuf)
{
	...
	if ((uintptr_t)pbuf & (CONFIG_SYS_CACHELINE_SIZE-1)) {
		align_length = (uintptr_t)pbuf & (CONFIG_SYS_CACHELINE_SIZE-1);
