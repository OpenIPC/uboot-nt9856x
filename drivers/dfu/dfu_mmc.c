...
#ifdef CONFIG_ARCH_NOVATEK
	static u64 last_buf_len = 0;
	/* Write to EMMC when size is more than 0x1600000 */
	/* FIXME The size cannot exceed 0x1600000, because DFU will time out after 5 seconds */
	if (dfu_file_buf_len < 0x1600000 && dfu_file_buf_len != 0) {
		if ( last_buf_len != dfu_file_buf_len ) {
			last_buf_len = dfu_file_buf_len;
			return 0;
		}
		else {
			last_buf_len = 0; // Receive 0 size data is mean finish
		}
	}
#endif
