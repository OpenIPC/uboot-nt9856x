...
#ifdef CONFIG_ARCH_NOVATEK
static int nvt_dfu_flush(struct dfu_entity *dfu, void *buf, int size, int blk_seq_num)
{
	int ret = 0;
...
#ifdef CONFIG_ARCH_NOVATEK
	if (dfu->i_buf - dfu->i_buf_start >= _EMBMEM_BLK_SIZE_ ) {
		ret = nvt_dfu_flush(dfu, buf, dfu->i_buf - dfu->i_buf_start, blk_seq_num);
		if (ret < 0) {
			dfu_transaction_cleanup(dfu);
			return ret;
		}
