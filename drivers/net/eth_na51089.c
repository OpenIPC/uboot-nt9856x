 ...
 * na51089:
 *    NOVATEK's NA51089 chip. This configuration uses an AXI master/DMA bus, an
...
#define ETHVERSION "1.0.0.1"

#define SW_RESET_ADDRESS     (IOADDR_ETH_REG_BASE + 0x3800)
#define SW_RESET_MASK        0x00000002
#define PLLREGWR_MASK        0x00000008

#define SW_PLL_RESET_ADDRESS     (IOADDR_ETH_REG_BASE + 0x84)
#define SW_PLL_RESET_MASK        0x3FFFFFFF
#define JTAG_EN_MASK		 0x100000
...
#define phy_pll_sw_reset_enable() do { \
        unsigned long v; \
        v = readl((void *)SW_PLL_RESET_ADDRESS); \
        v &= SW_PLL_RESET_MASK; \
        writel(v, (void *)SW_PLL_RESET_ADDRESS);\
} while(0)

#define phy_pll_sw_reset_disable() do { \
        unsigned long v; \
        v = readl((void *)SW_PLL_RESET_ADDRESS); \
        v |= ~SW_PLL_RESET_MASK; \
        writel(v, (void *)SW_PLL_RESET_ADDRESS);\
...
static void eqos_probe_hwinit(void)
{
	...
	*(uint32_t*) (IOADDR_TOP_REG_BASE + 0x1000) &= ~0x40000; /*Release SRAM shutdown*/
	...
	// phy setting
	mdelay(20);
	phy_pll_sw_reset_enable();
    mdelay(20);
    phy_pll_sw_reset_disable();
    mdelay(20);
	eth_phy_poweron();
	phy_sw_reset_enable();
			...
			*(uint32_t*) (IOADDR_CG_REG_BASE + 0x78) |= (1<<24);  /*Enable phyclk*/
				...
				*(uint32_t*) (IOADDR_TOP_REG_BASE + 0x18) |= 0x10000000;  /*Enable Pinmux*/
				if (phy_intf & PIN_ETH_CFG_RMII) {
					*(uint32_t*) (IOADDR_TOP_REG_BASE + 0xE8) &= ~0x400;       /*Enable Pinmux*/
				} else if (phy_intf & PIN_ETH_CFG_RMII_2) {
					*(uint32_t*) (IOADDR_TOP_REG_BASE + 0xA8) &= ~0x80;       /*Enable Pinmux*/
				}

			}
				if (phy_intf & (PIN_ETH_CFG_RMII)) {
					// reset external phy
					gpio_request(DSI_GPIO(6), "PHY_RST");
					...
					gpio_free(DSI_GPIO(6));

				} else if (phy_intf & (PIN_ETH_CFG_RMII_2)) {

					gpio_request(P_GPIO(11), "PHY_RST");
...
static void mac_pinmux_en(u32 phy_intf)
{
	if (phy_intf & PIN_ETH_CFG_RMII) {
			...
			*(uint32_t*) (IOADDR_TOP_REG_BASE + 0x14) &= ~0xC000000C;      /*Enable Pinmux*/
			*(uint32_t*) (IOADDR_TOP_REG_BASE + 0x14) |= 0x40000004;      /*Enable Pinmux*/
			*(uint32_t*) (IOADDR_TOP_REG_BASE + 0xE8) &= ~0x3BF;   /*Enable Pinmux*/
			*(uint32_t*) (IOADDR_TOP_REG_BASE + 0xE8) |= 0x40;   /*Enable Pinmux*/

			*(uint32_t*) (IOADDR_PAD_REG_BASE + 0x98) |= 0x400;   /*DSIGPIO5 8mA RFE_CLK50M*/

	} else if (phy_intf & PIN_ETH_CFG_RMII_2) {
			printf("phy interface: RMII_2\n");
			*(uint32_t*) (IOADDR_TOP_REG_BASE + 0x14) &= ~0xC000000C;      /*Enable Pinmux*/
			*(uint32_t*) (IOADDR_TOP_REG_BASE + 0x14) |= 0x80000008;      /*Enable Pinmux*/
			*(uint32_t*) (IOADDR_TOP_REG_BASE + 0xA8) &= ~0x73F;   /*Enable Pinmux*/
			*(uint32_t*) (IOADDR_TOP_REG_BASE + 0xA8) |= 0x800;   /*Enable Pinmux*/

			*(uint32_t*) (IOADDR_PAD_REG_BASE + 0x50) |= 0x10000;   /*PGPIO8 8mA REF_CLK50M*/
	}
	if (phy_intf & PIN_ETH_CFG_INTERANL) {
			...
			*(uint32_t*) (IOADDR_TOP_REG_BASE + 0x18) |= 0x10000000;  /*Enable Pinmux*/
			if (phy_intf & PIN_ETH_CFG_RMII) {
					...
					*(uint32_t*) (IOADDR_PAD_REG_BASE + 0x50) |= 0x04000;   /*PGPIO7 8mA EXT_REF_CLK25M*/
			}
	}
}
...
static void eth_parse_phy_intf(void)
{
	...
	if (phy_intf & 0x10) {
		...
		*(uint32_t*) (IOADDR_TOP_REG_BASE + 0x18) |= 0x2000;     /*Enable LED 1*/
		...
		reg = readl(IOADDR_TOP_REG_BASE + 0x04);
		if(reg & JTAG_EN_MASK)
			printf("===== JTAG_EN is enable. LED2 can not use. ===== \n");
...
int na51089_eth_initialize(bd_t *bis)
{
	struct eth_device *dev;
