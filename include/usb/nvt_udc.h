...
struct nvt_udc_regs {
	...
	uint32_t ep0_data; /* 0x128: EP0 Setup Packet PIO Register */
	uint32_t rsvd6[1];
	...
	uint32_t rsvd7[1];
	uint32_t ep[15]; /* 0x160 - 0x19B: Endpoint Register */
	uint32_t rsvd8[3];
	uint32_t fifocfg0; /* 0x1a8: Buffer Configuration 0 Register */
	uint32_t fifocfg1; /* 0x1aC: Buffer Configuration 1 Register */
	uint32_t fifocsr[4]; /* 0x1b0 - 0x1bf: FIFO Control Status Register */
	uint32_t dma_fifo; /* 0x1c0: DMA Target FIFO Register */
	uint32_t rsvd9[1];
	uint32_t dma_ctrl; /* 0x1c8: DMA0 Control Register */
	uint32_t dma_addr; /* 0x1cc: DMA0 Address Register */
	uint32_t rsvd10[1];
	uint32_t dma0_status; /* 0x1d4: DMA0 Controller Status Register */
	uint32_t dma1_status; /* 0x1d8: DMA1 Controller Status Register */
	uint32_t dma2_status; /* 0x1dc: DMA2 Controller Status Register */
	uint32_t dma3_status; /* 0x1e0: DMA3 Controller Status Register */
	uint32_t dma4_status; /* 0x1e4: DMA4 Controller Status Register */
	uint32_t rsvd11[6];
	uint32_t dma0_ctrl; /* 0x200: DMA1 Control Register */
	uint32_t dma0_addr; /* 0x204: DMA1 Address Register */
	uint32_t dma1_ctrl; /* 0x208: DMA2 Control Register */
	uint32_t dma1_addr; /* 0x20C: DMA2 Address Register */
	uint32_t dma2_ctrl; /* 0x210: DMA3 Control Register */
	uint32_t dma2_addr; /* 0x214: DMA3 Address Register */
	uint32_t dma3_ctrl; /* 0x218: DMA4 Control Register */
	uint32_t dma3_addr; /* 0x21C: DMA4 Address Register */
};

struct nvt_udc_ext_regs {
	/* USB Host Controller */
	struct {
		uint32_t data[4];
	} hccr;			/* 0x00 - 0x0f: hccr */
...
#ifdef CONFIG_USB_GADGET_CXSETUP_DMA
	uint32_t ep0_setup_data; /* 0x1d0: EP0 Setup Packet PIO Register */
#endif
};
...
#define CXFIFO_EXT_FIFOE(x)  (1 << (((x) & 0x03) + 8)) /* EPx FIFO empty */
#define CXFIFO_EXT_FIFOE_MASK (0x0f << 8)
...
#define GIMR1_MASK          0xffffff
#define GIMR1_EXT_MASK      0xf00ff

/* Group Interrupt Mask(Disable) Register 2 (Device) */
#define GIMR2_WAKEUP        (1 << 14) /* Device waked up */
#define GIMR2_IDLE          (1 << 13) /* Device idle */
#define GIMR2_DMA5FIN       (1 << 12) /* DMA 5 finished */
#define GIMR2_DMA4FIN       (1 << 11) /* DMA 4 finished */
#define GIMR2_DMA3FIN       (1 << 10) /* DMA 3 finished */
#define GIMR2_DMA2FIN       (1 << 9)  /* DMA 2 finished */
#define GIMR2_DMA1FIN       (1 << 8)  /* DMA 1 finished */
#define GIMR2_DMA0FIN       (1 << 7)  /* DMA 0 finished */
...
#define GIMR2_MASK          0x3ffff

#define GIMR2_EXT_WAKEUP    (1 << 10) /* Device waked up */
#define GIMR2_EXT_IDLE      (1 << 9)  /* Device idle */
#define GIMR2_EXT_DMAERR    (1 << 8)  /* DMA error */
#define GIMR2_EXT_DMAFIN    (1 << 7)  /* DMA finished */
#define GIMR2_EXT_MASK      0x7ff
...
#define GISR1_IN_FIFO(x)    (1 << (((x) & 0x0f) + 16))    /* FIFOx IN */
#define GISR1_OUT_FIFO(x)   (1 << (((x) & 0x0f) * 2))     /* FIFOx OUT */
#define GISR1_SPK_FIFO(x)   (1 << (((x) & 0x0f) * 2 + 1)) /* FIFOx SPK */
#define GISR1_RX_FIFO(x)    (3 << (((x) & 0x0f) * 2))     /* FIFOx OUT/SPK */

#define GISR1_EXT_IN_FIFO(x)  (1 << (((x) & 0x03) + 16))    /* FIFOx IN */
#define GISR1_EXT_OUT_FIFO(x) (1 << (((x) & 0x03) * 2))     /* FIFOx OUT */
#define GISR1_EXT_SPK_FIFO(x) (1 << (((x) & 0x03) * 2 + 1)) /* FIFOx SPK */
#define GISR1_EXT_RX_FIFO(x)  (3 << (((x) & 0x03) * 2))     /* FIFOx OUT/SPK */

/* Group Interrupt Status Register 2 (Device) */
#define GISR2_WAKEUP        (1 << 14) /* Device waked up */
#define GISR2_IDLE          (1 << 13)  /* Device idle */
#define GISR2_DMA5FIN       (1 << 12)  /* DMA finished */
#define GISR2_DMA4FIN       (1 << 11)  /* DMA finished */
#define GISR2_DMA3FIN       (1 << 10)  /* DMA finished */
#define GISR2_DMA2FIN       (1 << 9)  /* DMA finished */
#define GISR2_DMAERR        (1 << 8)  /* DMA error */
#define GISR2_DMA0FIN       (1 << 7)  /* DMA finished */
...
#define GISR2_EXT_WAKEUP    (1 << 10) /* Device waked up */
#define GISR2_EXT_IDLE      (1 << 9)  /* Device idle */
#define GISR2_EXT_DMAERR    (1 << 8)  /* DMA error */
#define GISR2_EXT_DMAFIN    (1 << 7)  /* DMA finished */
...
/* Endpoint Register */
#define EP_SENDZLP         (1 << 15)     /* Send Zero-Length-Packet */
#define EP_TNRHB(x)        (((x) & 0x03) << 13) \
	/* Transaction Number for High-Bandwidth EP(ISOC) */
#define EP_RESET           (1 << 12)     /* Reset Toggle Sequence */
#define EP_STALL           (1 << 11)     /* Stall */
...
#define FIFOCFG_EXT_EN      (1 << 5)
#define FIFOCFG_EXT_CFG_MASK   0x3f
#define FIFOCFG_EXT(fifo, cfg) (((cfg) & 0x3f) << (((fifo) & 3) << 3))

/* Buffer Map Register */
#define BUFFERMAP_BIDIR       (2 << 5)
#define BUFFERMAP_IN          (1 << 5)
#define BUFFERMAP_OUT         (0 << 5)
#define BUFFERMAP_DIR_MASK    0x60


/* FIFO Configuration Register */
#define FIFOCFG_EN              (1 << 7)
...
#define FIFOCFG_CFG_MASK        0xff
#define FIFOCFG(fifo, cfg)      (((cfg) & 0xff) << (((fifo) & 3) << 3))
#define FIFOCFG_DIR(fifo, dir)  (((dir) & 0x3f) << (fifo*8 + 5))

/* FIFO Control Status Register */
#define FIFOCSR_RESET          ((1 << 12) | (1 << 28)) /* FIFO Reset */
#define FIFOCSR_EVEN_BYTES(x)  ((x) & 0x7ff)           /* Length(bytes) for EVEN OUT-EP/FIFO */
#define FIFOCSR_ODD_BYTES(x)   (((x) >> 16) & 0x7ff)   /* Length(bytes) for ODD OUT-EP/FIFO */
#define FIFOCSR_EXT_RESET      (1 << 12) /* FIFO Reset */
#define FIFOCSR_BYTES(x)       ((x) & 0x7ff) /* Length(bytes) for OUT-EP/FIFO */

/* DMA Target FIFO Register */
#ifdef CONFIG_USB_GADGET_CXSETUP_DMA
#define DMAFIFO_CX_NO       0x10
#define DMAFIFO_CX          (1 << 16) /* DMA FIFO = CX FIFO */
#define DMAFIFO_FIFO(x)     (1 << ((x) & 0x1f)) /* DMA FIFO = FIFOx */
#else
